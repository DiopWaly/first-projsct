...
//    	double angle = Math.atan((targetX-currentX )/(targetY-currentY))*360/(2*Math.PI) - currentHeading;
...
//        throw new RuntimeException("implement me!");
    ...
    public static void drawPersonalArt(Turtle turtle) {
        ...
        for(PenColor color : PenColor.values()){
        	turtle.color(color);
	        for(int a = sides; a > 0; a--){
	            for(int b = 20; b>0; b--){
	                drawRegularPolygon(turtle, 5, b);
	            }
	            
	            drawRegularPolygon(turtle, 25, 10);
	            turtle.forward(sideLength);
